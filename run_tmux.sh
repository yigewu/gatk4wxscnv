...
bashCMD="tmux new-session -d -s "${toolDirName}"_"${scriptName}"_"${t}"_"${c}" 'docker run -v "${mainRunDir}":"${mainRunDir}" -v "${bamDir}":"${bamDir}" -w "${workDir}" "${imageName}" "${binaryFile}" "${scriptDir}${scriptName}${arg}" |& tee "${mainRunDir}"logs/"${scriptName}"_"${t}"_"${c}".log'" 
echo $bashCMD
#$bashCMD
